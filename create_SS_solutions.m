...
ParSize = 1; %adjust number of Parameter sets
tspan = 0:1:3000;
...
for i1 = 1:ParSize
    ...
    while value == 0
        [p,par] = Sample_avg(1,1); %samples one parameter set from case gg = 1
        ...
        for af = 0:2 %loop through different regulations
            ...
            if af == 0
               J = [(eG*kcatG*m)/(KmG + m)^2 - (eL*kcatL)/(KmL + m) - (eG*kcatG)/(KmG + m) + (eL*kcatL*m)/(KmL + m)^2 + (eL*kcatL*m^2)/(alpha*(KmL + m)^2) - (2*eL*kcatL*m)/(alpha*(KmL + m)), - (kcatL*m)/(KmL + m) - (kcatL*m^2)/(alpha*(KmL + m)),            -(kcatG*m)/(KmG + m);
