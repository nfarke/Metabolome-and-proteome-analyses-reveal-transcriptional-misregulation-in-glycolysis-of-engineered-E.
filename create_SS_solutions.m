...
for i1 = 1:ParSize
    ...
    while value == 0
        ...
        for af = 0:2 %loop through different regulations
            ...
            par(strcmp('kcatL',p)) = (par(strcmp('rU',p)) - 0.01)./(par(strcmp('eL',p))/(1 + par(strcmp('KmL',p)))); 
            par(strcmp('betaL',p)) = par(strcmp('eL',p)) *0.01;
            ...
            alphax = par(find(strcmp(p,'alphax')),1);
            ...
            mue  =  (kcatL * eL * m/(m + KmL))/alphax;
            
            %Mass balances
            dmdt = rU - kcatL * eL * m/(m + KmL) - kcatG * eG * m/(m + KmG) - mue*m;
            ...
            if af == 0
               J = [(eG*kcatG*m)/(KmG + m)^2 - (eL*kcatL)/(KmL + m) - (eG*kcatG)/(KmG + m) + (eL*kcatL*m)/(KmL + m)^2 + (eL*kcatL*m^2)/(alphax*(KmL + m)^2) - (2*eL*kcatL*m)/(alphax*(KmL + m)), - (kcatL*m)/(KmL + m) - (kcatL*m^2)/(alphax*(KmL + m)),            -(kcatG*m)/(KmG + m);
                    (eL^2*kcatL*m)/(alphax*(KmL + m)^2) - (eL^2*kcatL)/(alphax*(KmL + m)),                     -(2*eL*kcatL*m)/(alphax*(KmL + m)),                               0;
                    (eG*eL*kcatL*m)/(alphax*(KmL + m)^2) - (eG*eL*kcatL)/(alphax*(KmL + m)),                       -(eG*kcatL*m)/(alphax*(KmL + m)), -(eL*kcatL*m)/(alphax*(KmL + m))];

            elseif af == 1
               J = [(eG*kcatG*m)/(KmG + m)^2 - (eL*kcatL)/(KmL + m) - (eG*kcatG)/(KmG + m) + (eL*kcatL*m)/(KmL + m)^2 + (eL*kcatL*m^2)/(alphax*(KmL + m)^2) - (2*eL*kcatL*m)/(alphax*(KmL + m)), - (kcatL*m)/(KmL + m) - (kcatL*m^2)/(alphax*(KmL + m)),            -(kcatG*m)/(KmG + m)
                                                                                a1*betaL*m^(a1 - 1) - (eL^2*kcatL)/(alphax*(KmL + m)) + (eL^2*kcatL*m)/(alphax*(KmL + m)^2),                     -(2*eL*kcatL*m)/(alphax*(KmL + m)),                               0
                                                                                                    (eG*eL*kcatL*m)/(alphax*(KmL + m)^2) - (eG*eL*kcatL)/(alphax*(KmL + m)),                       -(eG*kcatL*m)/(alphax*(KmL + m)), -(eL*kcatL*m)/(alphax*(KmL + m))];

            elseif af == 2
              J = [(eG*kcatG*m)/(KmG + m)^2 - (eL*kcatL)/(KmL + m) - (eG*kcatG)/(KmG + m) + (eL*kcatL*m)/(KmL + m)^2 + (eL*kcatL*m^2)/(alphax*(KmL + m)^2) - (2*eL*kcatL*m)/(alphax*(KmL + m)), - (kcatL*m)/(KmL + m) - (kcatL*m^2)/(alphax*(KmL + m)),            -(kcatG*m)/(KmG + m)
                                                                                a1*betaL*m^(a1 - 1) - (eL^2*kcatL)/(alphax*(KmL + m)) + (eL^2*kcatL*m)/(alphax*(KmL + m)^2),                     -(2*eL*kcatL*m)/(alphax*(KmL + m)),                               0
                                                                          a2*betaG*ind*m^(a2 - 1) - (eG*eL*kcatL)/(alphax*(KmL + m)) + (eG*eL*kcatL*m)/(alphax*(KmL + m)^2),                       -(eG*kcatL*m)/(alphax*(KmL + m)), -(eL*kcatL*m)/(alphax*(KmL + m))];
        ...
        %only continue if all three models satisfy the conditions
...
%save stable parameter sets in PAR.mat
